defmodule Pxblog.CommentControllerTest do
  ...
  test "creates resource and redirects when data is valid", %{conn: conn, post: post} do
    ...
    get conn, user_post_path(conn, :show, post.user, post)
  ...
  test "deletes the comment when logged in as an authorized user", %{conn: conn, post: post, comment: comment, user: user} do
