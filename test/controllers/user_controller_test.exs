defmodule Pxblog.UserControllerTest do
  ...
  @tag admin: true
  test "redirect from new form when not admin", %{conn: conn, nonadmin_user: nonadmin_user} do
  ...
  @tag admin: true
  test "redirect from creating user when not admin", %{conn: conn, user_role: user_role, nonadmin_user: nonadmin_user} do
    conn = login_user(conn, nonadmin_user)
    conn = post conn, user_path(conn, :create), user: valid_create_attrs(user_role)
    assert get_flash(conn, :error) == "You are not authorized to create new users!"
    assert redirected_to(conn) == page_path(conn, :index)
  ...
  @tag admin: true
  test "redirects away from editing chosen resource when logged in as a different user", %{conn: conn, nonadmin_user: nonadmin_user, nonadmin_user2: nonadmin_user2} do
    conn = login_user(conn, nonadmin_user)
    conn = get conn, user_path(conn, :edit, nonadmin_user2)
  ...
  @tag admin: true
  test "does not update chosen resource and redirects when data is valid and logged in as different admin", %{conn: conn, nonadmin_user: nonadmin_user, nonadmin_user2: nonadmin_user2} do
    conn = login_user(conn, nonadmin_user)
    conn = put conn, user_path(conn, :update, nonadmin_user2), user: @valid_create_attrs
    assert get_flash(conn, :error) == "You are not authorized to modify that user!"
    assert redirected_to(conn) == page_path(conn, :index)
    assert conn.halted
  end
